/** Seed the Firestore emulator with the §9 dev dataset. */
...
function emptyBreakfast() {
  return { entree: null, all: [] };
}

async function main(): Promise<void> {
  const stamp = admin.firestore.FieldValue.serverTimestamp();
  const docs: Record<string, Record<string, unknown>> = {
    // No-school weekday with the HAR-verified closure label.
    "2026-09-07": {
    ...
    // Plain A day with HAR entrée names (lunch 09/08 + breakfast 09/08).
    "2026-09-08": {
      ...
      lunch: {
        ...
        soups: [], sides: [],
        all: ["Italian-Roasted Pork Loin", "Pho Bar"],
      },
      breakfast: { entree: "Grilled Ham", all: ["Grilled Ham", "Cheddar Scrambled Eggs"] },
      sources: { icalUid: "seed-uid-a", sageWeek: "09/06/2026" }, updatedAt: stamp,
    },
    // Special B day with HAR entrée names (lunch 09/09 + breakfast 09/09).
    "2026-09-10": {
      date: "2026-09-10", dow: "Thu", abc: "B", isSpecial: true,
      ...
      breakfast: { entree: "Bacon", all: ["Bacon", "Sticky Rice", "Pineapple Cup"] },
      sources: { icalUid: "seed-uid-special", sageWeek: "09/06/2026" }, updatedAt: stamp,
    },
    // Plain C day with HAR entrée names (lunch 09/11 + breakfast 09/11).
    "2026-09-11": {
      date: "2026-09-11", dow: "Fri", abc: "C", isSpecial: false, specialLabel: null,
      isNoSchool: false, noSchoolLabel: null,
      lunch: {
      ...
      breakfast: { entree: "Grilled Ham", all: ["Grilled Ham", "Greek Scrambled Eggs"] },
      sources: { icalUid: "seed-uid-c", sageWeek: "09/06/2026" }, updatedAt: stamp,
    },
    // Weekend doc: exists in Firestore but must never be navigable/rendered.
    "2026-09-12": {
