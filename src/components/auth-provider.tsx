...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInGoogle = async () => {
    ...
    // Hint the hosted domain; rules enforce lakesideschool.org regardless.
    provider.setCustomParameters({ hd: "lakesideschool.org" });
  ...
  const devSignIn = async () => {
    ...
    const res = await fetch("/api/dev/token", {
      ...
      body: JSON.stringify({ email: "tester@lakesideschool.org" }),
    });
  ...
  const schoolUser = !!user?.email?.endsWith("@lakesideschool.org");
