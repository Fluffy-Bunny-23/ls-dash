...
tmux -S ~/tmux/sockets/agents send-keys -t ls-dash:emulators \
  'cd <repo> && export JAVA_HOME=~/coding/tmp/jre/usr/lib/jvm/java-21-openjdk-arm64 && export PATH="$JAVA_HOME/bin:$PATH" && export GCLOUD_PROJECT=demo-ls-dash && firebase emulators:start --only auth,firestore --project demo-ls-dash' Enter
...
cd <repo> && export FIRESTORE_EMULATOR_HOST=127.0.0.1:8081 \
  FIREBASE_AUTH_EMULATOR_HOST=127.0.0.1:9090 FIREBASE_PROJECT_ID=demo-ls-dash \
  CRON_SECRET=dev-only-secret-change-in-prod \
...
LSDASH_BASE=https://3000.dev.oc.zwjde.com node scripts/browser-check.mjs  # vs public URL
```
...
- App: `https://3000.dev.oc.zwjde.com` → VM port 3000
- Auth emulator: `https://9090.dev.oc.zwjde.com` → VM port 9090
- Firestore emulator: `https://8081.dev.oc.zwjde.com` → VM port 8081
...
- Emulator-minted **custom tokens drop `email`/`email_verified` claims** —
  ...
  via Admin SDK (`/api/dev/token` provisions `tester@lakesideschool.org`;
